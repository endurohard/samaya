...
//
// Оттенок берётся не из всего круга: полный диапазон 0–360° давал кислотные
// синие и салатовые, которые рядом с бордо выглядят чужеродно. Ограничиваем
// тёплым сектором (охра → терракота → бордо → слива) и снижаем насыщенность,
// чтобы аватары не спорили с фирменным цветом.
const AVATAR_HUES = [8, 22, 34, 96, 168, 208, 262, 340];

export function stringToColor(s) {
  let h = 0;
  for (let i = 0; i < (s || '').length; i++) h = (h * 31 + s.charCodeAt(i)) >>> 0;
  const hue = AVATAR_HUES[h % AVATAR_HUES.length];
  // Лёгкий разброс светлоты внутри одного тона — иначе одинаковых кружков
  // рядом становится слишком много.
  const light = 42 + ((h >> 3) % 3) * 6;
