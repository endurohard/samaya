...
Кнопка «Каталоги по ссылке» в разделе «Услуги» открывает модалку `catalogsModal` с двумя
панелями (список / редактор) — без стопки модалок, как у «Должностей». Создание каталога сразу
переводит в редактор: там ссылка `/c/<token>` с копированием и QR (через `qrcode`, как у акций),
переключатель «Ссылка активна», «Сменить ссылку», и список всех активных услуг с галочками,
сгруппированный по группам услуг, с поиском по названию. Отмеченные получают номер `#n` —
это порядок в каталоге (порядок добавления, хранится в `catalogSel`). Сохранение — `PATCH`
каталога + `PUT .../services` с полным составом.

Первой строкой в списке закреплён **каталог сайта** `/services` — у него нет токена и своего
состава: редактор в режиме `catalogMode = 'site'` показывает те же галочки, но отмеченные = услуги
с `show_in_menu`, без номеров порядка (порядок на сайте задают разделы). Сохранение шлёт
`PATCH /api/salons/services/:id { show_in_menu }` только по изменившимся услугам.

nginx: `location /c/` → `/api/salons/public/site/c/` (SSR в [[salon-service]]).
