...
См. [[../decisions/2026-07-13-security-correctness-audit]]. `SAVEPOINT` в consume-worker
против «отравленного» события при общем расходнике (C2); агрегация потребления по продукту;
дедуп виртуальных движений `stock_insufficient` (L6, миграция 029); склад по умолчанию по
`is_default` (M13); RBAC fail-closed (M8).
